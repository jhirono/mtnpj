...
def process_batch_results(results) -> Dict[str, Dict]:
    """Process batch results and return dict mapping route_id to tags"""
    route_tags = {}
    
    if not results:
        print("Warning: No results received from API")
        return route_tags
    ...
    for result in results:
        try:
            # Get route ID from custom_id
            route_id = result.get('custom_id')
            if not route_id:
                print(f"Skipping result with no custom_id: {result}")
                continue
                
            # Navigate to the content containing the tags
            content = result.get('response', {}).get('body', {}).get('choices', [{}])[0].get('message', {}).get('content')
            if not content:
                print(f"No content found for route {route_id}")
                continue
                
            try:
                # Parse the content string into JSON
                tags_data = json.loads(content)
                print(f"Processing tags for route {route_id}: {tags_data}")
                
                # Extract the llm_tags field and ensure values are lists
                llm_tags = {}
                for category, tags in tags_data.get('llm_tags', {}).items():
                    # If tags is empty list or None, store empty list
                    if not tags:
                        llm_tags[category] = []
                    # If tags is a string, convert to list
                    elif isinstance(tags, str):
                        llm_tags[category] = [tags]
                    # If tags is already a list, use as is
                    elif isinstance(tags, list):
                        llm_tags[category] = tags
                    
                route_tags[route_id] = llm_tags
                print(f"Successfully processed tags for route {route_id}")
                
            except json.JSONDecodeError as e:
                print(f"Error parsing tags JSON for route {route_id}: {e}")
                continue
    ...
    print(f"Successfully processed tags for {len(route_tags)} routes")
    return route_tags
...
def process_routes(input_file: str, prompt_file: str, retrieve_only: bool = False, batch_id: str = None):
    """Main processing function with separate submit and retrieve operations"""
    # Load data and prompt
    with open(input_file, 'r') as f:
        data = json.load(f)
    
    if retrieve_only and batch_id:
        # Only retrieve and process results
        ...
        # First apply manual tagging
        data = manual_tagging(data)
        
        # Then update routes with LLM tags and combine tags
        routes_updated = 0
        for area in data:
            for route in area.get('routes', []):
                route_id = route.get('route_id')
                if route_id in route_tags:
                    # Initialize llm_tags if not present
                    if 'llm_tags' not in route:
                        route['llm_tags'] = {}
                    # Update with new LLM tags
                    route['llm_tags'] = route_tags[route_id]
                    
                    # Combine manual and LLM tags into route_tags
                    manual_tags = route.get('manual_tags', {})
                    llm_tags = route['llm_tags']
                    
                    # Create new combined tags dictionary
                    combined_tags = {}
                    
                    # First add all LLM tags
                    combined_tags.update(llm_tags)
                    
                    # Then add manual tags, mapping categories as needed
                    for manual_category, tags in manual_tags.items():
                        llm_category = map_category(manual_category)
                        if llm_category in combined_tags:
                            # Get existing tags for this category
        ...
        print(f"Updated {routes_updated} routes with combined tags")
        
        # Write to new output file with _tagged suffix
        ...
        batch_requests = create_batch_requests(data, prompt_template)
        
        if not batch_requests:
            print("No routes to process")
            return data
...
def submit_batch_only(routes: List[Dict], prompt_template: str) -> str:
    """Submit a batch and return the batch ID"""
    batch_requests = create_batch_requests(routes, prompt_template)
...
if __name__ == "__main__":
    ...
    prompt_file = "prompt/route_prompt_simple.txt"
    
    if len(sys.argv) == 3:
        # Retrieve mode
        batch_id = sys.argv[2]
        process_routes(input_file, prompt_file, retrieve_only=True, batch_id=batch_id)
    else:
        # Submit mode
        process_routes(input_file, prompt_file)