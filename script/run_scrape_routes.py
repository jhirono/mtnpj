...
def main(input_file):
    ...
    all_areas = []
    for index, url in enumerate(area_urls, start=1):
        try:
            print(f"Scraping {url} ({index}/{len(area_urls)})...")
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Usage: python3 scrape_routes_from_area_combined.py <input_json_file>")
    else:
